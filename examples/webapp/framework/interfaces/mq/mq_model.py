...
from typing import Optional, NoReturn, Coroutine, Any

from aio_pika import connect_robust, ExchangeType, IncomingMessage
from aio_pika.abc import AbstractRobustChannel, AbstractRobustConnection, AbstractRobustQueue, AbstractQueue
...
class MQModel(metaclass=ABCMeta):
    def __init__(self):
        ...
        if not self._configs:
            raise Exception("Don't find amqp configuration. Check app_config.yml")
    ...
    async def declare_topic(self, channel: AbstractRobustChannel) -> AbstractQueue:
        """
    ...
    async def declare_queue(self, channel: AbstractRobustChannel):
        ...
        :return: Объявленная очередь
        """
        self._queue = self.config.get("consumeQueue")
        queue = await channel.declare_queue(self._queue)

        return queue

    async def get_queue(self, channel: AbstractRobustChannel) -> Coroutine[Any, Any, AbstractQueue]:
        """
    ...
    async def give_data(self, message: MQMessage, *args, **kwargs) -> NoReturn:
        ...
        try:
            await self._publish_data(
                message.result, message.headers,
